...
36. Gate project- and org-scoped assignment and unassignment on current `org_membership`, require the custom role's owning `org_id` to match the target project/org, and initially suppress assignments without membership during permission resolution. Membership removal explicitly deletes every project- and org-scoped assignment before deleting the membership, in one transaction. This part is complete; the later RBAC query restructuring makes membership and tenant ownership structural assignment invariants, so canonical permission resolution can rely on valid assignments rather than compensate for dangling rows. Depends on 18, 33, 35.
...
38. Apply the same superset check to revoke, custom-role deletion, and permissions added to or removed from a custom role. Deletion requires organization- or system-scoped authority over every permission carried by the role because it removes the role from every project- and organization-scoped assignment. This part is complete. Depends on 32, 37.
...
57. Project-scoped resource isolation: when the first ordinary project-owned resource is introduced, filter every core-layer store method by both resource ID and project ID (`WHERE id = @id AND project_id = @project_id`). Add RLS with `FORCE ROW LEVEL SECURITY`, keyed on `app.project_id`, and a real-Postgres test using the runtime app role that proves a row from another project is invisible. Apply the same review checklist to every later project-scoped resource.
58. Protected bootstrap identities: control projects are already protected from deletion and from changing `is_control`. When an API first allows deleting or renaming an organization or project, add the remaining database backstops required for protected bootstrap identities and test them through that API.
...
- `scripts/seed-dev-operator.sh` (`make seed-dev-operator`) seeds a local-only bootstrap operator user (`operator@theapp.com`) and grants it `superadmin` at system scope, so every `cmd/cli` command has a valid `--operator`. The assignment remains a direct SQL insert even though `role assign-system` exists: that mutating command itself requires an existing operator, while this script creates the first one. It's a standing dev convenience, not a task with its own checkpoint; revisit it whenever a phase changes what a fresh local environment needs to be immediately useful.
