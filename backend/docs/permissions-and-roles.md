...
Deleting a user is a soft delete: the `users` table gets a `deleted_at` column, set on delete rather than removing the row outright. Before setting it, the delete operation takes the same global system-role-management lock used by system-role revocation and verifies that another active user will still hold every registered permission through system-scoped assignments. All of the target user's assignments are excluded from that check because a soft-deleted user cannot authenticate or act as the recovery administrator. This check and the soft delete happen in one transaction.

Global user deletion and organization membership removal are separate lifecycle operations. The system-level user delete disables the identity across every organization and requires `user:delete`. Removing a user from one organization instead deletes that organization's project- and organization-role assignments before deleting its membership, leaving the global identity and memberships in other organizations active. Removing the final organization membership does not implicitly delete the global user.

Creating a user never silently restores a soft-deleted identity. Both direct user creation and organization-user creation report that the email belongs to a deleted user. Restoration is an explicit system-level operation guarded by `user:restore`; it clears `deleted_at`, updates `updated_at`, rotates the `etag`, and makes all retained memberships and role assignments effective again.

A soft-deleted user's rows in `project_role_assignments`, `org_role_assignments`, and `system_role_assignments` are left in place rather than cleaned up as part of the delete itself — unlike role or project/org deletion above, there's no correctness problem in leaving them, since permission resolution and the global recovery check exclude any user with `deleted_at` set regardless of what their assignment rows say, the same way authentication itself would reject that user before assignments ever come into play.
