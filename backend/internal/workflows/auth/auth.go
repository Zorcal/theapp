...
func NewWorkflowCore(core *coreauth.Core, emailSender email.Sender, cfg coreauth.Config, dbosCtx dbos.DBOSContext) *WorkflowCore {
	return &WorkflowCore{
		core:        core,
...
// RequestMagicLink sends a sign-in link to emailAddr durably. If the process crashes after
// the token is stored but before the email is sent, DBOS resumes from the email step on restart.
// If ctx carries a workflow ID (set by the idempotency interceptor from x-idempotency-key metadata),
// DBOS deduplicates on it so retrying with the same key returns the original result without sending
// a second email.
...
func (w *WorkflowCore) requestMagicLinkWorkflow(ctx dbos.DBOSContext, emailAddr string) (struct{}, error) {
	...
	if _, err = dbos.RunAsStep(ctx, func(ctx context.Context) (struct{}, error) {
		return struct{}{}, w.sendMagicLinkEmail(ctx, emailAddr, rawToken)
	}, dbos.WithStepName("send-email")); err != nil {
