...
A consequence of the split: `internal/core/rbac.Core`'s custom-role operations (`UpdateRole`, `DeleteRole`, `AssignRole`, `RolePermissions`, ...) resolve a role by external ID against `rbac.custom_roles` only. A static role's external ID simply isn't a row there, so passing one in gets the same `mdl.ErrNotFound` as any other nonexistent or cross-org ID, not a distinct "that's a static role" error.
