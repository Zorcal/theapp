...
**There is no "project membership" table.** A user's access to a project is derived from role assignments owned by `rbac`: a `project_role_assignments` row grants access to that one project directly; an `org_role_assignments` row grants access to every project under that org; a `system_role_assignments` row grants access to every project system-wide. Project- and org-scoped custom-role grants are active only while the user has current membership in the owning organization.
