...
//
// SystemRoleService assigns and unassigns system roles to users, and lists the system roles
// available to assign. System roles are statically seeded rather than managed through an API, so
// this service has no endpoints for creating, updating, or deleting one. Every call is anchored on
// the "theapp" organization's control project: x-project-id must be that project's ID.
