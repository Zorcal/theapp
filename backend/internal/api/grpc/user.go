...
func (s *userService) ListUsers(ctx context.Context, req *pb.ListUsersRequest) (*pb.ListUsersResponse, error) {
	...
	pageToken, err := conv.DecodePageToken[mdl.UserOrderByField](req.GetPageToken())
	if err != nil {
	...
	// Validate caller hasn't changed sorting mid-pagination.
	if req.GetPageToken() != "" {
		if !slices.Equal(pageToken.OrderBys, orderBys) {
	...
	if nextPageOffset < totalCount {
		nextPageToken, err = conv.EncodePageToken(nextPageOffset, orderBys)
		if err != nil {
