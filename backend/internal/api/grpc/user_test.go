...
func TestUserService_DeleteUser(t *testing.T) {
	srvTest := NewServerTest(t, ServerConfig{
		Log: testingx.NewLogger(t),
		UserCore: &MockedUserCore{
			DeleteUserFunc: func(_ context.Context, _ uuid.UUID) error { return nil },
		},
	})

	if _, err := srvTest.userServiceClient.DeleteUser(authCtxForTestUser(t, t.Context()), &pb.DeleteUserRequest{
		Id: uuid.NewString(),
	}); err != nil {
...
func TestUserService_DeleteUser_error(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name:     "validated request",
			userCore: &MockedUserCore{},
			in:       &pb.DeleteUserRequest{Id: "bad"},
			want:     codes.InvalidArgument,
		},
		{
			name: "active user not found",
			userCore: &MockedUserCore{
				DeleteUserFunc: func(_ context.Context, _ uuid.UUID) error { return mdl.ErrNotFound },
			},
			in:   &pb.DeleteUserRequest{Id: uuid.NewString()},
			want: codes.NotFound,
		},
		{
			name: "last fully privileged system administrator",
			userCore: &MockedUserCore{
				DeleteUserFunc: func(_ context.Context, _ uuid.UUID) error {
		...
		},
		{
			name: "core error",
			userCore: &MockedUserCore{
				DeleteUserFunc: func(_ context.Context, _ uuid.UUID) error { return errors.New("boom") },
			},
			in:   &pb.DeleteUserRequest{Id: uuid.NewString()},
			want: codes.Internal,
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := srvTest.userServiceClient.DeleteUser(authCtxForTestUser(t, t.Context()), tt.in)
			if status.Code(err) != tt.want {
				t.Errorf("DeleteUser() error code = %v, want %v", status.Code(err), tt.want)
			}
		})
	}
}
