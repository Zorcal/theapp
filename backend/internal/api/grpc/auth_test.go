...
func TestAuthService_VerifyMagicLink(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			authCore: &MockedAuthCore{
				VerifyMagicLinkFunc: func(_ context.Context, _ string) (mdl.TokenPair, error) {
			...
			want: &pb.TokenPair{AccessToken: "access-token", RefreshToken: "refresh-token", ExpiresIn: 900},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestAuthService_VerifyMagicLink_error(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			authCore: &MockedAuthCore{
				VerifyMagicLinkFunc: func(_ context.Context, _ string) (mdl.TokenPair, error) {
					return mdl.TokenPair{}, errors.New("boom")
				},
			},
			in:   &pb.VerifyMagicLinkRequest{Token: "sometoken"},
			want: status.New(codes.Internal, "Internal"),
...
func TestAuthService_RefreshAccessToken(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			authCore: &MockedAuthCore{
				RefreshAccessTokenFunc: func(_ context.Context, _ string) (mdl.TokenPair, error) {
					return mdl.TokenPair{
			...
			want: &pb.TokenPair{AccessToken: "new-access", RefreshToken: "new-refresh", ExpiresIn: 900},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got, err := srvTest.authServiceClient.RefreshAccessToken(srvTest.authCtx(t, t.Context()), tt.in)
			if err != nil {
...
func TestAuthService_RefreshAccessToken_error(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := srvTest.authServiceClient.RefreshAccessToken(srvTest.authCtx(t, t.Context()), tt.in)
			if err == nil {
