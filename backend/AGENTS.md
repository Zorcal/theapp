...
Call the function under test directly. When another function is only used for setup, call it through a `seedX` or `mustX`
test helper so the subject of the test remains clear.
